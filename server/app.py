...
app.config['MAIL_SERVER'] = 'smtp.gmail.com'  # Replace with your email service provider
...
app.config['MAIL_DEFAULT_SENDER'] = os.getenv('MAIL_USERNAME')  # Default sender address
...
def create_appointment():
    ...
    # Determine clinic email based on location
    clinic_email = get_clinic_email(clinic_location)

    if not clinic_email:
        return jsonify({"message": "Invalid clinic location"}), 400

    # Prepare email content
    subject = f"New Appointment Request - {pet_id}"
    body = f"""
    ...
    try:
        msg = Message(subject, recipients=[clinic_email], body=body)
        mail.send(msg)
        return jsonify({"message": "Appointment submitted successfully"}), 200
    except Exception as e:
        return jsonify({"message": f"Failed to send email: {str(e)}"}), 500

# Function to get clinic email based on location
def get_clinic_email(location):
    clinic_emails = {
        "Downtown": "namada871@gmail.com"
    }
    return clinic_emails.get(location)
